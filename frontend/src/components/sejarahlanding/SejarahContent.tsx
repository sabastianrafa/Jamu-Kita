...
import { sejarahArticles } from "@/data/SejarahArticles"; // ⬅️ DATA STATIC DIPISAHKAN
...
export default function SejarahContent() {
  ...
  const openArticle = (article: typeof sejarahArticles[0]) => {
      ...
      {sejarahArticles.map((article, idx) => (
        ...
        >
          ...
          <div className="flex-1">
            ...
            <div
              className={`${
                idx % 2 === 1 ? "flex justify-end md:justify-start" : ""
              }`}
            >
              <button
